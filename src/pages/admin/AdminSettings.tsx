...
export default function AdminSettings() {
  ...
  const [evaluationReminders, setEvaluationReminders] = useState(false);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          ...
          <div className="space-y-4">
            <ToggleRow label="Email notifications" enabled={emailNotifications} onToggle={setEmailNotifications} />
            <ToggleRow label="Absence alerts" enabled={absenceAlerts} onToggle={setAbsenceAlerts} />
            <ToggleRow label="Report reminders" enabled={reportReminders} onToggle={setReportReminders} />
            <ToggleRow label="Evaluation due reminders" enabled={evaluationReminders} onToggle={setEvaluationReminders} />
          </div>
        </div>
        ...
        <button
          ...
          onClick={() =>
            toast({
              ...
              description: "Configuration updates have been applied.",
            })
          }
          className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-accent text-accent-foreground text-sm font-medium hover:opacity-90 transition-opacity"
        >
...
function ToggleRow({ label, enabled, onToggle }: { label: string; enabled: boolean; onToggle: (enabled: boolean) => void }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-border last:border-0">
      <span className="text-sm text-muted-foreground">{label}</span>
      <button
        ...
        className={`w-9 h-5 rounded-full transition-colors cursor-pointer ${enabled ? 'bg-accent' : 'bg-muted'}`}
      >
